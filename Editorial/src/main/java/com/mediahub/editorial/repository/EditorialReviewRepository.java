...
public class EditorialReviewRepository {
    ...
    // API 3 — GET — Find by ID
    public EditorialReview findById(int reviewID) {
        String sql = "SELECT * FROM EditorialReview WHERE ReviewID = ?";
    ...
    // API 4, 5, 6 — POST — Update decision
    public int updateDecision(int reviewID, String decision, String remarks, String status) {
        String sql = "UPDATE EditorialReview SET Decision=?, Remarks=?, " +
                     "ReviewDate=NOW(), Status=? WHERE ReviewID=?";
        return jdbcTemplate.update(sql, decision, remarks, status, reviewID);
    }
}
