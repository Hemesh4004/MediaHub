...
public class EditorialReviewService {
    ...
    // API 1 — Submit content for review
    public Map<String, Object> submitReview(EditorialReview review) {
        ...
        if (result > 0) {
            ...
            response.put("message", "Content submitted for review successfully");
    ...
    public EditorialReview getReviewById(int reviewID) {
        return repository.findById(reviewID);
    }

    // API 4 — Approve review
    public Map<String, Object> approveReview(int reviewID, String remarks) {
        Map<String, Object> response = new HashMap<>();
        int result = repository.updateDecision(reviewID, "Approved", remarks, "Completed");
        if (result > 0) {
            response.put("reviewID", reviewID);
            response.put("decision", "Approved");
            response.put("status", "Completed");
            response.put("message", "Content approved successfully. Ready for scheduling.");
    ...
    // API 5 — Reject review
    public Map<String, Object> rejectReview(int reviewID, String remarks) {
        Map<String, Object> response = new HashMap<>();
        int result = repository.updateDecision(reviewID, "Rejected", remarks, "Completed");
        if (result > 0) {
            response.put("reviewID", reviewID);
            response.put("decision", "Rejected");
            response.put("status", "Completed");
            response.put("message", "Content rejected. Creator has been notified.");
            response.put("statusCode", 200);
        } else {
            response.put("error", "Review not found");
            response.put("statusCode", 404);
    ...
    public Map<String, Object> requestRevision(int reviewID, String remarks) {
        Map<String, Object> response = new HashMap<>();
        int result = repository.updateDecision(reviewID, "RevisionRequired", remarks, "Pending");
        if (result > 0) {
            response.put("reviewID", reviewID);
            response.put("decision", "RevisionRequired");
            response.put("status", "Pending");
            response.put("message", "Revision requested. Creator has been notified to resubmit.");
            response.put("statusCode", 200);
        } else {
            response.put("error", "Review not found");
            response.put("statusCode", 404);
